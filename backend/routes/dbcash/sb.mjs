...
/*Routes are named after positions at a poker table. SB stands for Small Blind.
  The Small Blind position is, clockwise, before the Big Blind.
  The Small Blind is the first (smaller) mandatory bet at a regular Texas Hold'em (and Omaha too) table.
  #8 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "SB" collection from the "dbCash" database.
  let collection = await dbCash.collection("SB");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("SB");
