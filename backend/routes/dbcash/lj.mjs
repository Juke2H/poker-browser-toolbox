...
/*Routes are named after positions at a poker table. LJ stands for Lojack (or lowjack).
  Also called the Middle Position in some notations.
  The Lojack position is, clockwise, before the Hijack.
  #4 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "LJ" collection from the "dbCash" database.
  let collection = await dbCash.collection("LJ");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("LJ");
