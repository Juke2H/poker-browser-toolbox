...
/*Routes are named after positions at a poker table. UTG stands for Under The Gun.
  The UTG position is, clockwise, before the UTG +1.
  #1 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "UTG" collection from the "dbCash" database.
  let collection = await dbCash.collection("UTG");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("UTG");
