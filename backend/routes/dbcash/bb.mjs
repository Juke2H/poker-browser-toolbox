...
/*Routes are named after positions at a poker table. BB stands for Big Blind
  The Big Blind position is, clockwise, after Small Blind and before Under the Gun.
  The Big Blind is the second (bigger) mandatory bet at a regular Texas Hold'em (and Omaha) table.
  #9 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "BB" collection from the "dbCash" database.
  let collection = await dbCash.collection("BB");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("BB");
