...
/*Routes are named after positions at a poker table. CO stands for Cutoff.
  The Cutoff position is, clockwise, before the Button.
  #6 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "CO" collection from the "dbCash" database.
  let collection = await dbCash.collection("CO");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("CO");
