...
/*Routes are named after positions at a poker table. 
  UTG+1 (or UTG1 because I don't want a + in the URL) stands for Under The Gun plus one.
  The UTG+1 position is, clockwise, before the MP (or UTG+2).
  #2 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "UTG1" collection from the "dbCash" database.
  let collection = await dbCash.collection("UTG1");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("UTG1");
