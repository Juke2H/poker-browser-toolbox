...
/*Routes are named after positions at a poker table. MP stands for Middle Position.
  Also called Under the Gun + 2 in some notations; I prefer using Lojack and having only one UTG plus position.
  The Middle Position is, clockwise, before the Lojack.
  #3 at a 9-handed table. */
...
router.get("/", async (req, res) => {
  // Access the "MP" collection from the "dbCash" database.
  let collection = await dbCash.collection("MP");
...
router.delete("/:id", async (req, res) => {
  ...
  let collection = dbCash.collection("MP");
